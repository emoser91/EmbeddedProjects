...
There are two different examples of a linked list below. 
Make sure to comment out one or the other to run. 
...
/*
	Here is another example but this one was generated by OpenAI GPT when asked: write a linked list in cabs
	
	A linked list is a data structure that consists of a sequence of nodes, where each node contains a value 
	and a pointer to the next node in the sequence. Here is a simple implementation of a linked list in C:
	
	
	This program defines a struct Node that represents a node in the linked list. It then uses this struct to 
	create two nodes, with values 5 and 10, and link them together to form a linked list. Finally, it uses a 
	while loop to traverse the list and print out the values of the nodes.

	Note that this program uses dynamic memory allocation to create the nodes, which is necessary because the 
	number of nodes in a linked list can vary. It also does not include any functions for adding or removing 
	nodes from the list, which would typically be included in a more complete implementation of a linked list.
